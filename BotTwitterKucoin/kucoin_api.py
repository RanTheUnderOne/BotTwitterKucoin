...
class kucoin_api:
    ...
    def __init__(self, api_keys, logfile=False, block=False, account_json=None):
        self.api_keys = {'api_key': api_keys['kucoin_keys']['api_key'],
                         'secret_key': api_keys['kucoin_keys']['secret_key']}
    ...
    def log_trade(self, ticker, buy_volume, hold_times, buy_trade, sell_trades, gain_text, status, simulate):
        ...
        self.started_time.strftime('%Y-%m-%d_%H-%M-%S'), self.account_json, 'simulation' if simulate else 'live'),
                  "a") as log_name:
            ...
            if type(status) == dict:
                json.dump({'url': status['url'], 'update_text': status['update_text'],
                           ...
                           'hold_times': hold_times, 'complete_time': now, 'buy_volume': buy_volume, 'buy': buy_trade,
                           'sell': sell_trades, 'telegram': gain_text}, log_name)
