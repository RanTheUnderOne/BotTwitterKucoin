        ...
        try:
            ...
            if self.coin_subset:
                ...
                for coin_pair in self.coin_subset:
                    ...
                    self.markets = list(
                        filter(lambda x: x['id'] == coin_pair.replace('/', ''), self.exchange.fetch_markets()))

                # Add exchange rate with base coin and USDT
                if self.base_coin != 'USDT':
                    try:
                        self.all_tickers[self.base_coin + '/USDT'] = self.exchange.fetch_ticker(
                            self.base_coin + '/USDT')
    ...
    def buy_sell_volumes(self, buy_dollars, interval):  # FIND OUT ABOUT LEVERAGED LIMITS AND SELL INCREMENTS
        ...
        while 1:
            ...
            if self.count_pulls % 10 == 0:
                print('Pulled live prices (updates every 20 mins), there are %d tradeable tickers with %s - %s' % (
                len(self.cryptos), self.base_coin, datetime.now().strftime('%m/%d - %H:%M:%S')))
