...
class Listener():
    ...
    def on_status(self, status):
        if str(status.user.id_str) in self.ids:
            ...
            print('\n\n\n%s: %s \n\n%s %s' % (
            datetime.now().strftime('%H:%M:%S'), full_text, status.user.screen_name, status.user.id_str))
            print(status.created_at)
            if any(word in full_text.lower() for word in self.keywords) and status.in_reply_to_status_id is None:
                print('\n\nMoonshot Inbound!\n\n')

                # Execute trade
                self.exchange.execute_trade(self.pair, hold_times=self.hold_time, buy_volume=self.buy_volume,
