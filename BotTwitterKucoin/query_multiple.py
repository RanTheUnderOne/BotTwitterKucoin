...
class Twitter_Query:
    def __init__(self, api, users, sell_coin, hold_times, buy_volume, simulate, exchange, exchange_data, buy_coin=None,
                 log_file=None, full_ex=True, cancel=[False]):
    ...
    def parse_tweet(self, status, utc_time):
        ...
        successful = False
        if any(substr in full_text.lower() for substr in
               self.users[status.user.screen_name]['keywords']) and utc_time - status.created_at < timedelta(
                seconds=10):
            ...
            if self.buy_coin:
                ...
                try:
                    ...
                    t = threading.Thread(target=self.exchange.execute_trade, args=(pair,),
                                         kwargs={'hold_times': self.hold_times, 'buy_volume': coin_vol,
                                                 'simulate': self.simulate, 'status': status})
                    t.start()
        ...
        print('\n\n' + '-' * 15 + ' New Tweet ' + '-' * 15)
        print('%s\n@%s - %s:\n\n"%s"' % (
        datetime.now().strftime('%H:%M:%S'), status.user.screen_name, status.created_at.strftime('%b %d at %H:%M:%S'),
        full_text))
...
def query_tweets(api, users, sell_coin, hold_times, buy_volume, simulate, exchange, print_timer=False, log_file=None,
                 buy_coin=None, full_ex=True, exchange_data=None, cancel=[False]):
    ...
    try:
        # Calculate delay
        delay = len(users)
        querys = Twitter_Query(api, users, sell_coin, hold_times, buy_volume, simulate, exchange, exchange_data,
                               log_file=log_file, full_ex=full_ex, cancel=cancel)

        # Check for tweets from a user
        for user, v in users.items():
            # Create a thread for each user here
            t = threading.Thread(target=querys.query, args=({'username': user, 'id': v['id']}, delay),
                                 kwargs={'print_timer': print_timer})
            t.start()
